...
def get_documents_from_source_knowledge_base():
    source_dify = DifyPlatform(config.api_config('prod'), config.s3_config('prod'), dify_db_name='prod')
...
def sync_documents_to_target_knowledge_base():
    source_dify = DifyPlatform(config.api_config('prod'), config.s3_config('prod'), dify_db_name='prod')
    target_dify = DifyPlatform(config.api_config('dev'), config.s3_config('dev'), dify_db_name='dev')
    for kb_mapping in config.mapping:
...
def replace_images_in_target_knowledge_base_documents():
    ...
    for kb_mapping in config.mapping:
        ...
        if source_docs_with_images:
            ...
            for item in source_docs_with_images:
                item['image'] = [local_images[uuid] for uuid in item['image'] if uuid in local_images]
            target_images = target_dify.upload_images_to_dify(source_docs_with_images, target_kb)
